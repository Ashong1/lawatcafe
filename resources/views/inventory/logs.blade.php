...
<div class="bg-[#FDF8F5] min-h-screen -m-6 p-6 md:p-8 text-[#4A3B32]" style="font-family: 'Montserrat', sans-serif;">
    ...
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-[#F0E6D2] mb-6">
        <form action="{{ route('inventory.logs') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                <label for="log-filter-ingredient" class="block text-[10px] font-black text-[#8D6E63] uppercase tracking-widest mb-2 ml-1">Ingredient</label>
                <select id="log-filter-ingredient" name="ingredient_id" class="w-full bg-[#FAFAFA] border border-[#F0E6D2] rounded-xl px-4 py-2 text-xs font-bold text-[#3E2723] focus:border-[#3E2723] focus:ring-0 transition-all">
            ...
            <div>
                <label for="log-filter-user" class="block text-[10px] font-black text-[#8D6E63] uppercase tracking-widest mb-2 ml-1">Admin User</label>
                <select id="log-filter-user" name="user_id" class="w-full bg-[#FAFAFA] border border-[#F0E6D2] rounded-xl px-4 py-2 text-xs font-bold text-[#3E2723] focus:border-[#3E2723] focus:ring-0 transition-all">
            ...
            <div>
                <label for="log-filter-date-from" class="block text-[10px] font-black text-[#8D6E63] uppercase tracking-widest mb-2 ml-1">Date From</label>
                <input type="date" id="log-filter-date-from" name="date_from" value="{{ request('date_from') }}" class="w-full bg-[#FAFAFA] border border-[#F0E6D2] rounded-xl px-4 py-2 text-xs font-bold text-[#3E2723] focus:border-[#3E2723] focus:ring-0 transition-all">
            </div>
            <div class="flex items-end gap-2">
                <div class="flex-1">
                    <label for="log-filter-date-to" class="block text-[10px] font-black text-[#8D6E63] uppercase tracking-widest mb-2 ml-1">Date To</label>
                    <input type="date" id="log-filter-date-to" name="date_to" value="{{ request('date_to') }}" class="w-full bg-[#FAFAFA] border border-[#F0E6D2] rounded-xl px-4 py-2 text-xs font-bold text-[#3E2723] focus:border-[#3E2723] focus:ring-0 transition-all">
                </div>
                <button type="submit" class="bg-[#3E2723] text-white p-2.5 rounded-xl hover:bg-[#271815] transition shadow-sm active:scale-95">
                    <x-lucide-search class="w-4 h-4" />
                </button>
                <a href="{{ route('inventory.logs') }}" class="bg-[#FAFAFA] text-[#8D6E63] p-2.5 rounded-xl border border-[#F0E6D2] hover:bg-[#FDF8F5] transition active:scale-95" title="Clear Filters" aria-label="Clear Filters">
                ...
                <tbody class="text-sm">
                    ...
                    <tr class="border-b border-[#FAFAFA] group hover:bg-[#FDF8F5]/50 transition-colors">
                        <td class="py-4 text-[#8D6E63] text-xs font-medium">
                            ...
                            <span class="block text-[10px] text-[#6D4C41]">{{ $log->created_at->format('h:i A') }}</span>
                            <span class="block text-[10px] text-[#6D4C41] font-medium md:hidden">By: {{ $log->user->name ?? 'System' }}</span>
                        </td>
                        <td class="py-4 font-bold text-[#3E2723]">
                            {{ $log->ingredient->name ?? 'N/A' }}
                            <span class="block text-[10px] text-[#6D4C41] font-medium md:hidden normal-case">{{ $log->reason }}</span>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="py-16 text-center">
                            <div class="flex flex-col items-center opacity-30">
                                ...
                                <p class="text-[#6D4C41] text-sm font-medium">No inventory logs found matching your filters.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
