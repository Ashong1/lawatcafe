            ...
            <div class="space-y-8">
                ...
                <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-[#F0E6D2]">
                    <div class="flex items-center gap-3 mb-8">
                        ...
                        <div>
                            <h3 class="text-sm font-black text-[#3E2723] uppercase tracking-widest">Gmail IMAP Audit</h3>
                            <p class="text-[10px] text-[#A1887F] font-medium italic">Credentials for automated GCash/Maya receipt parsing.</p>
                        </div>
                    </div>

                    <div class="space-y-6">
                        <div>
                            <label class="block text-[10px] font-black text-[#3E2723] uppercase mb-2 tracking-widest">Gmail Username</label>
                            <input type="email" name="imap_username" value="{{ $settings['imap_username'] }}" class="w-full bg-[#FDF8F5] border-2 border-[#F0E6D2] rounded-xl px-4 py-3 text-sm font-bold focus:outline-none focus:border-blue-500 transition-all">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-[#3E2723] uppercase mb-2 tracking-widest">Gmail App Password</label>
                            <div x-data="{ show: false }" class="relative">
                                <input :type="show ? 'text' : 'password'" name="imap_password" value="{{ $settings['imap_password'] }}" class="w-full bg-[#FDF8F5] border-2 border-[#F0E6D2] rounded-xl px-4 py-3 text-sm font-bold focus:outline-none focus:border-blue-500 transition-all">
                            ...
                            <p class="text-[9px] text-[#8D6E63] mt-2 italic">Do not use your main account password. Generate an "App Password" in Google Account Security.</p>
                        </div>
                    </div>
                </div>

                <div class="bg-blue-600 p-6 md:p-8 rounded-3xl shadow-xl text-white">
                    <div class="flex items-center gap-3 mb-6">
                        <x-lucide-shield-check class="w-6 h-6 text-blue-200" />
                        <h3 class="text-sm font-black uppercase tracking-widest">Integration Security</h3>
                    </div>
                    <p class="text-xs leading-relaxed text-blue-100 font-medium">
                        Communication with Gmail uses SSL/TLS encryption. Reference numbers extracted from emails are cross-referenced with your local database to prevent double-claiming of vouchers.
                    </p>
                </div>
            </div>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-[#F0E6D2]">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="active_ai_model" class="w-full bg-[#FDF8F5] border-2 border-[#F0E6D2] rounded-xl px-4 py-3 text-sm font-bold focus:outline-none focus:border-[#3E2723] transition-all appearance-none cursor-pointer">
                                    <option value="gemini-1.5-pro" {{ $settings['active_ai_model'] === 'gemini-1.5-pro' ? 'selected' : '' }}>Gemini 1.5 Pro (Best for Complex Regex/Parsing)</option>
                    ...
                    <p class="text-[10px] text-[#8D6E63] mt-8 italic leading-relaxed border-t border-[#F0E6D2] pt-4">
                        The AI engine analyzes your sales trends every hour and provides feedback via the floating chat interface. It also serves as the primary fallback for payment extraction if standard Regex patterns fail to read receipt emails.
                    </p>
                </div>
